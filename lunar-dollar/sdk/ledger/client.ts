...
export class CantonLedgerClient implements LedgerConnection {
   ...
   * Template IDs are automatically resolved from package names to hashes.
   * @param templateId - Template ID (package name format)
   * @param contractId - Contract ID to exercise on
   * @param choice - Choice name
   * @param args - Choice arguments
   * @param additionalActors - Additional parties to act as (for multi-party choices)
   */
  async exercise<T, R>(
    templateId: string,
    ...
    choice: string,
    args: unknown,
    additionalActors?: Party[]
  ): Promise<ExerciseResult<R>> {
    // Resolve package name to hash
    const resolvedTemplateId = await this.resolveTemplateId(templateId);

    // Build actAs list: always include this.party, plus any additional actors
    // Use Set to deduplicate in case of overlap
    const actAsSet = new Set<Party>([this.party]);
    if (additionalActors) {
      additionalActors.forEach(p => actAsSet.add(p));
    }
    const actAs = Array.from(actAsSet);
