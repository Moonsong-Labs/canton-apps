...
export function LunarDollarBalance() {
  ...
  // Query TransferableFungible holdings (what LNRD uses)
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          ...
          {balances.length > 0 && (
            <Card>
              <h3 className="text-lg font-semibold text-slate-200 mb-4">All Holdings</h3>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-700">
                      <th className="text-left py-3 px-4 text-sm font-medium text-slate-400">Instrument</th>
                      <th className="text-right py-3 px-4 text-sm font-medium text-slate-400">Balance</th>
                      <th className="text-right py-3 px-4 text-sm font-medium text-slate-400">Holdings</th>
                    </tr>
                  </thead>
                  <tbody>
                    {balances.map((balance) => (
                      <tr key={`${balance.instrumentId}-${balance.instrumentVersion}`} className="border-b border-slate-700/50">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <span className="text-xl">{balance.instrumentId === 'LNRD' ? '🌙' : '💰'}</span>
                            <div>
                              <p className="font-medium text-slate-200">{balance.instrumentId}</p>
                              <p className="text-xs text-slate-500">v{balance.instrumentVersion}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-3 px-4 text-right">
                          <p className="font-mono text-lg text-slate-200">
                            {balance.totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </p>
                        </td>
                        <td className="py-3 px-4 text-right text-slate-400">
                          {balance.holdingCount}
                        </td>
                      </tr>
                    ))}
                  </tbody>
